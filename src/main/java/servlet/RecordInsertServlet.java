...
public class RecordInsertServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(productId.equals("") || amount == null || amount.equals("") || customer == null) {
			session.setAttribute("resultMsg", "エラー。商品を選択していません、または数量を入力していません。");
			RequestDispatcher dispatcher = request.getRequestDispatcher("WEB-INF/jsp/recordInsert.jsp");
			dispatcher.forward(request, response);
		...
		if (stock.getProductStock() < amountInt) {
			session.setAttribute("resultMsg", productId + "の在庫が足りません。");
			RequestDispatcher dispatcher = request.getRequestDispatcher("WEB-INF/jsp/recordInsert.jsp");
			dispatcher.forward(request, response);
			session.removeAttribute("resultMsg");
		} else {
